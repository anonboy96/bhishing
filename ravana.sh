...
up_date(){
	echo -ne ""
	curl https://raw.githubusercontent.com/anonboykrvert/Ravana/master/version --output vs > /dev/null 2>&1
	ver=$(cat vs)
	if [[ $ver == "V.3.0.0" ]];then
	echo -ne ""
	else 
	...
	git pull https://github.com/anonboykrvert/Ravana.git
	fi 
	rm -rf vs > /dev/null 
}
...
about_me(){
	...
	echo ""
	echo -e "${g}[${w}+${g}] ${y} I am anonboy kumar and i am a junior IT engineer.\n"
	echo -e "${g}[${w}01${g}] ${p} Youtube: https://in.linkedin.com/in/umang-patel-514517257 "
	echo -e "${g}[${w}02${g}] ${p} Instagram: https://www.instagram.com/anonboy.96 "
	echo -e "${g}[${w}03${g}] ${p} Facebook: https://btechkers.netlify.app "
	read ab_optn
	if [[ $ab_optn == "01" || $ab_optn == "1" ]];then
		xdg-open https://in.linkedin.com/in/umang-patel-514517257 > /dev/null 2>&1
	elif [[ $ab_optn == "02" || $ab_optn == "2" ]];then
		xdg-open https://www.instagram.com/anonboy.96 > /dev/null 2>&1
	elif [[ $ab_optn == "03" || $ab_optn == "3" ]];then
		xdg-open https://btechkers.netlify.app > /dev/null 2>&1
	else 
	echo -ne "Invalid option"
	fi
	else
	echo " "
	echo -e "${g}[${w}+${g}] ${y} I am Umang Patel and i am an IT Student.\n"
	echo -e "${g}[${w}01${g}] ${p} Linkedin"
	echo -e "${g}[${w}02${g}] ${p} Instagram "
	echo -e "${g}[${w}03${g}] ${p} Btechkers"
	echo -e "${r} Choose 🤜 "
	read ab_optn
	if [[ $ab_optn == "01" || $ab_optn == "1" ]];then
		am start -a android.intent.action.VIEW -d https://in.linkedin.com/in/umang-patel-514517257 > /dev/null 2>&1
	elif [[ $ab_optn == "02" || $ab_optn == "2" ]];then
		am start -a android.intent.action.VIEW -d https://www.instagram.com/anonboy.96 > /dev/null 2>&1
	elif [[ $ab_optn == "03" || $ab_optn == "3" ]];then
		am start -a android.intent.action.VIEW -d https://btechkers.netlify.app > /dev/null 2>&1
	else 
		echo -e "${r} Invalid option 🥵 "
fi
fi
...
banner(){
	...
	printf "
       ...
       ╦═╗╔═╗╦  ╦╔═╗╔╗╔╔═╗
       ╠╦╝╠═╣╚╗╔╝╠═╣║║║╠═╣ \e[0;1m MADE BY anonboy
       ╩╚═╩ ╩ ╚╝ ╩ ╩╝╚╝╩ ╩  V3.0.0\e[32;1m 
       Instagram : anonboy.96                                             
